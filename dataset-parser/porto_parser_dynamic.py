...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Porto dataset parser.')
    ...
    for dataset_name in ["porto_output_res7.csv", "porto_output_res8.csv", "porto_output_res9.csv"]:
        time_interval = 15  # The time interval between each record in the dataset
        ...
        def process(row):
            ...
            x = [item[0]
                 for item in x for i in range(item[1])]  # Expand counts
            res = dict()
            for item in range(len(x)):
                # Calculate the window for each item
                window = (time+item*time_interval)//window_size
                res[window] = x[item]  # Store location and window
            return res.items()
        ...
        data['time'] = data['count'].apply(lambda x: int(x[0]))
