...
class Trainer:
    ...
    def __init__(self, model: torch.nn.Module, dataset: TrajectoryBatchDataset, config: dict,
        ...
        self.lr_decay_iters = self.lr_decay_epochs * len(self.train_dataset)

        self.validation_dataset = TrajectoryBatchDataset(
            os.path.join(config["data_dir"], config["dataset"], f"resolution-{config['resolution']}"),
            dataset_type='val',
    ...
    def train(self):
        ...
        self.logger.info(f"Starting training, Train batches: {len(self.train_dataset)}, Val batches: {len(self.validation_dataset)}")
        ...
        for epoch in range(self.max_epochs):
            ...
            for X, Y in (p_bar := tqdm(self.train_dataset, leave=False)):
                iter_num += 1
                ...
                p_bar.set_postfix({'loss': total_loss / total_samples})
    ...
    def save_checkpoint(self):
        ...
        try:
            ...
            self.logger.info(f"Saved current best model to {checkpoint_path}")
        except (OSError, RuntimeError) as e:
            self.logger.error(f"Failed to save checkpoint due to an error: {e}")
